...
public class AdminNavigationDataInitializer implements CommandLineRunner{
 ...
 private void normalize(){
  ...
  ensure(operation,"PG 운영","/admin/payment-operations","PG",1,AdminRole.USER);ensure(operation,"매출 원장","/admin/payment-operations/sales-ledger","₩",2,AdminRole.USER);ensure(operation,"미확정 매출","/admin/payment-operations/pending-sales","⏳",3,AdminRole.USER);ensure(operation,"매출 분석","/admin/payment-operations/sales-analytics","%",4,AdminRole.USER);ensure(operation,"PG 정산 대사","/admin/payment-operations/settlements/reconciliation","✓",5,AdminRole.USER);ensure(operation,"정산 관리","/admin/payment-operations/settlements","✓",6,AdminRole.USER);ensure(operation,"DB 명세","/admin/database-spec","DB",7,AdminRole.USER);
