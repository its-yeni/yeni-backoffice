(function(){
 ...
 $('ledger-start').value=initialParams.get('startDate')||(initialParams.get('period')==='today'?today(0):today(0));
 ...
 const resetBtn=$('ledger-reset');if(resetBtn)resetBtn.onclick=()=>{$('ledger-keyword').value='';$('ledger-start').value=today(0);$('ledger-end').value=today(0);$('ledger-type').value='';$('ledger-settlement').value='';if(confirmSel)confirmSel.value='';resetAndLoad();};
